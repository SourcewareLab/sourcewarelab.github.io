async function shiftLeftMobile() {
  ...
  await Promise.all([fadeLeftMobile(activeDiv.children[0]), fadeInLeftMobile(rightDiv.firstElementChild)]).then(() => {
  });
...
function fadeInLeftMobile(node) {
  let leftMargin = 0;
  let opacity = 0;
